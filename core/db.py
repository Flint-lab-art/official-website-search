...
_BASE_COLS = [
    ...
    ("baidu_shot", "TEXT"),
    ("bing_shot", "TEXT"),
    ("baidu_m_status", "TEXT NOT NULL DEFAULT 'pending'"),
    ("baidu_m_rank", "INTEGER"),
    ("baidu_m_page", "INTEGER"),
    ("baidu_m_evidence", "TEXT"),
    ("baidu_m_shot", "TEXT"),
    ("bing_m_status", "TEXT NOT NULL DEFAULT 'pending'"),
    ("bing_m_rank", "INTEGER"),
    ("bing_m_page", "INTEGER"),
    ("bing_m_evidence", "TEXT"),
    ("bing_m_shot", "TEXT"),
]
...
def _now():
    # astimezone() 补本地时区（DTZ005），strftime 输出与原来完全一致
...
def init_db(db_path):
    ...
    if "engines" not in cols:
        conn.execute(
            "ALTER TABLE tasks ADD COLUMN engines TEXT DEFAULT 'baidu,bing,baidu_m,bing_m'"
        )
    conn.commit()
    return conn
...
def ensure_keywords(conn, keywords, engines=None):
    ...
    for kw in keywords:
        ...
        conn.execute(
            ...
            "ON CONFLICT(keyword) DO UPDATE SET engines=excluded.engines, "
...
def update_result(conn, keyword, engine, status, rank=None, page=None, evidence=None, shot=None):
    ...
    conn.execute(
        f"UPDATE tasks SET {col[0]}=?, {col[1]}=?, {col[2]}=?, "  # noqa: S608
        f"{col[3]}=?, {col[4]}=?, updated_at=? WHERE keyword=?",
...
def retry_engine(conn, keyword, engine):
    ...
    conn.execute(
        f"UPDATE tasks SET {col}='pending', updated_at=? WHERE keyword=?",  # noqa: S608
        (_now(), keyword),
    )
    conn.commit()
...
def summary(conn):
    ...
    return {
        "baidu_hit": b_hit or 0,
        "bing_hit": g_hit or 0,
        "baidu_m_hit": bm_hit or 0,
        "bing_m_hit": gm_hit or 0,
        "total": total or 0,
    }
